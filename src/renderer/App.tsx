...
import { runSimulation, createUnit, getEntityName, getPack } from '../engine/index.js';
...
const UnitImage: React.FC<{ unit: Unit; name: string; size: number }> = ({ unit, name, size }) => {
  if (unit.imageUrl) {
    return <EntityImage imageUrl={unit.imageUrl} name={name} size={size} />;
  }
  if (unit.svgType) {
...
export function App({
  ...
  const { isRecording, startRecording, stopRecording, stats: captureStats } = useScreenRecorder('ffa');
  ...
  const startBattle = () => {
    ...
    const result = runSimulation(boardState, { width: layout.cols, height: layout.rows });
    setIsBattling(true);
    setWinner(null);
    setCurrentTick(0);
    setAbilityEffects([]);
  ...
  return (
    <div style={{
      ...
      <h1 style={{
        fontSize: TYPE.size.display - 6, marginBottom: SPACE.md,
        letterSpacing: TYPE.tracking.wide, color: COLORS.text.primary,
      }}>
        {pack.name.toUpperCase()} FFA
      </h1>
      ...
      <div style={{
        ...
        <div style={playAreaStyle(layout)}>
          ...
          {units.map(unit => {
            ...
            // A 10px lunge from a board-edge unit now spills into the arena
            // margin rather than out of the arena, so it no longer needs
            // suppressing at the edges.
            const lungeOffset = state.attacking ? (state.attackDir * layout.tile * 0.2) : 0;
            ...
            return (
              ...
              >
                ...
                {/* Unit image with attack flash, plus any active ability aura */}
                <div style={{
                  position: 'relative',
                  ...
                  {state.alive && <AbilityAura abilities={state.auras} size={layout.tile * 0.7} />}
                  <UnitImage unit={unit} name={displayName} size={layout.tile * 0.7} />
                </div>

                {/* HP bar */}
                <div style={{
                  position: 'absolute',
                  bottom: layout.tile * 0.04,
                  left: '10%',
                  width: '80%',
                  height: Math.max(3, layout.tile * 0.1),
                  background: '#222',
                  borderRadius: 3,
                  overflow: 'hidden',
